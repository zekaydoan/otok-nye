...
export default async function VehicleDetailPage({
  ...
  // olacağından burada strong consistency kullanıyoruz.
  ...
  const kmIssueRecordIds = new Set(kmIssues.map((i) => i.recordId));
    ...
    <div>
      ...
      {!isOwnVehicle && (
        <div className="mt-4 rounded-lg border-l-4 border-amber-400 bg-amber-50 p-3 text-sm text-amber-800">
          Bu araç sisteme <strong>{creatorShop?.name || "başka bir bayi"}</strong>{" "}
